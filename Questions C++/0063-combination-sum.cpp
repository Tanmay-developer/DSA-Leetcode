...
class Solution {
public:
    void solve(vector<int>& candidates, vector<vector<int>>& ans, int target, int pos, vector<int>& temp){
        ...
        if(candidates[pos] <= target){
            ...
            solve(candidates ,ans, target - candidates[pos], pos, temp);
            temp.pop_back();
        }
        solve(candidates,ans, target, pos+1, temp);
    }
