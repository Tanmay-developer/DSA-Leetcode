...
class Solution:
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        res = []
        temp = []
        self.solve(0, candidates, res, temp, target)
        return res
    
    def solve(self, idx, candidates, res, temp, target):
        ...
        if candidates[idx] <= target:
            ...
            self.solve(idx, candidates, res, temp, target-candidates[idx])
            temp.pop()
        
        self.solve(idx+1, candidates, res, temp, target)